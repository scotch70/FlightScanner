...
class AirportViewModel: ObservableObject {
    @Published var airports: [AirportsModel] = []
    ...
    var allflightData: URLComponents {
        get {
            URLComponents(string: api + FlightScannerViewModel.data[1] + apiKey)!
        }
    }
    ...
    func fetchFlightData(completion: @escaping (Result<AirportsModel , Error>) -> Void) {
    ...
    let task = URLSession.shared.dataTask(with: urlComponents.url!) {
        ...
        if let data = data {
            do {
                let flightData = try jsonDecoder.decode(AirportsModel.self, from: data)
